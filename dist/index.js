...
function render(root, ctx, data, loading, error, view) {
    ...
    if (backBtn) {
        backBtn.addEventListener('click', () => {
            if (view.kind === 'entry') {
                root._mmView = { kind: 'project', project: view.project };
                render(root, ctx, data, false, null, root._mmView);
            }
            else if (view.kind === 'project') {
                root._mmView = { kind: 'list' };
                render(root, ctx, data, false, null, root._mmView);
            }
        });
    }
    ...
    for (const el of Array.from(root.querySelectorAll('[data-project-name]'))) {
        el.addEventListener('click', () => {
            ...
            if (!project)
                return;
            root._mmView = { kind: 'project', project };
            render(root, ctx, data, false, null, root._mmView);
        });
    }
    // Restore scroll
...
async function loadEntry(root, ctx, data, project, entry) {
    try {
        const result = (await api.rpc('GET', `memory/entry/${encodeURIComponent(project.projectName)}/${encodeURIComponent(entry.file)}`));
        root._mmView = { kind: 'entry', project, entry, content: result.content };
        render(root, ctx, data, false, null, root._mmView);
    }
    catch (err) {
        const c = themeColors(ctx.theme === 'dark');
        render(root, ctx, data, false, err.message, root._mmView ?? { kind: 'list' });
    }
}
// ── Mount / Unmount ────────────────────────────────────────────────────
...
export function mount(container, pluginApi) {
    ...
    async function loadData() {
        ...
        loading = true;
        render(root, ctx, cached, loading, lastError, root._mmView ?? { kind: 'list' });
        try {
            const data = (await api.rpc('GET', 'memory'));
            cached = data;
            lastError = null;
            render(root, ctx, data, false, null, root._mmView ?? { kind: 'list' });
        }
        catch (err) {
            lastError = err.message;
            render(root, ctx, cached, false, lastError, root._mmView ?? { kind: 'list' });
        }
        finally {
            loading = false;
        }
