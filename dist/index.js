 ...
 * Manual refresh via ↻ button; no automatic polling.
 */
...
function render(root, ctx, data, loading, error, view) {
    ...
    if (backBtn) {
        backBtn.addEventListener('click', () => {
            const mm = root.parentNode;
            if (view.kind === 'entry') {
                mm._mmView = { kind: 'project', project: view.project };
                render(root, ctx, data, false, null, mm._mmView);
            }
            else if (view.kind === 'project') {
                mm._mmView = { kind: 'list' };
                render(root, ctx, data, false, null, mm._mmView);
            }
        });
    }
    // Wire entry clicks
    for (const el of Array.from(root.querySelectorAll('[data-project][data-file]'))) {
        el.addEventListener('click', () => {
            ...
            if (!entry)
                return;
            loadEntry(root, ctx, data, project, entry, root.parentNode);
        });
    }
    // Wire project clicks
    for (const el of Array.from(root.querySelectorAll('[data-project-name]'))) {
        el.addEventListener('click', () => {
            ...
            if (!project)
                return;
            const mm = root.parentNode;
            mm._mmView = { kind: 'project', project };
            render(root, ctx, data, false, null, mm._mmView);
        });
    }
    // Restore scroll
...
function renderRoot(c, data, loading, error, view) {
    const total = data?.totalEntries ?? 0;
    let headerTitle = 'Memory';
    if (view.kind === 'project')
        headerTitle = view.project.projectName;
    if (view.kind === 'entry')
        headerTitle = view.entry.name;
    let breadcrumb = '';
    if (view.kind === 'entry') {
        breadcrumb = `
    ...
    <div style="height:100%;display:flex;flex-direction:column;overflow:hidden">
      ...
      ">
        ...
        ${data ? `<span style="font-size:0.65rem;color:${c.muted}">${total} total · ${data.projects.length} projects</span>` : ''}
        <button id="mm-refresh-btn" style="
...
async function loadEntry(root, ctx, data, project, entry, mm) {
    try {
        const result = (await api.rpc('GET', `memory/entry/${encodeURIComponent(project.projectName)}/${encodeURIComponent(entry.file)}`));
        mm._mmView = { kind: 'entry', project, entry, content: result.content };
        render(root, ctx, data, false, null, mm._mmView);
    }
    catch (err) {
        const c = themeColors(ctx.theme === 'dark');
        render(root, ctx, data, false, err.message, mm._mmView ?? { kind: 'list' });
    }
}
// ── Mount / Unmount ────────────────────────────────────────────────────
let api;
export function mount(container, pluginApi) {
    ensureStyles();
    api = pluginApi;
    // Persist view state on container so it survives plugin remounts (e.g. panel switching)
    const mm = container;
    if (!mm._mmView)
        mm._mmView = { kind: 'list' };
    ...
    async function loadData() {
        ...
        render(root, ctx, cached, loading, lastError, mm._mmView ?? { kind: 'list' });
        try {
            const data = (await api.rpc('GET', 'memory'));
            cached = data;
            lastError = null;
            loading = false;
            render(root, ctx, data, false, null, mm._mmView ?? { kind: 'list' });
        }
        catch (err) {
            lastError = err.message;
            render(root, ctx, cached, false, lastError, mm._mmView ?? { kind: 'list' });
        }
        finally {
            loading = false;
        }
    ...
    root.addEventListener('click', (e) => {
        const btn = e.target.closest('#mm-refresh-btn');
        if (btn) {
            firstLoad = true;
            loadData();
        }
    ...
    const unsubscribe = api.onContextChange(() => {
        ...
        if (newProjectPath !== currentProjectPath) {
            // Project actually changed — reset to list view
            currentProjectPath = newProjectPath;
            mm._mmView = { kind: 'list' };
            firstLoad = true;
            loadData();
        }
        // If project unchanged, do nothing — preserve current view
    });
    container._mmUnsubscribe = unsubscribe;
}
export function unmount(container) {
    if (typeof container._mmUnsubscribe === 'function') {
        container._mmUnsubscribe();
        delete container._mmUnsubscribe;
    }
    container.innerHTML = '';
}
