...
function inferType(filename: string, description: string): MemoryEntry['type'] {
  ...
  if (lower.includes('user') || lower.includes('用户')) return 'user';
  return 'unknown';
}

function parseMemFilename(filename: string): string {
  return filename.replace(/\.md$/, '');
}

function parseMemoryIndex(content: string, dirPath: string): MemoryEntry[] {
...
function getMemoryData(): MemoryResponse {
  const projectsDir = path.join(os.homedir(), '.claude', 'projects');
  const projects: ProjectMemory[] = [];

  let projectDirs: string[] = [];
  try { projectDirs = fs.readdirSync(projectsDir); } catch { /* ignore */ }

  let totalEntries = 0;

  for (const dir of projectDirs) {
    const memDir = path.join(projectsDir, dir, 'memory');
    let memDirStat: fs.Stats;
    try { memDirStat = fs.statSync(memDir); } catch { continue; }
    if (!memDirStat.isDirectory()) continue;
    ...
    for (const file of files) {
      if (file === 'MEMORY.md' || !file.endsWith('.md') || indexedFiles.has(file)) continue;
      ...
      try {
        ...
        if (firstLine) { desc = firstLine.replace(/^#+\s*/, '').trim().slice(0, 120); }
      } catch { /* ignore */ }
...
function getEntryContent(projectName: string, file: string): EntryResponse | null {
...
const server = http.createServer((req: http.IncomingMessage, res: http.ServerResponse) => {
  ...
  if (req.method === 'GET' && req.url) {
    ...
    if (url.pathname === '/memory' || url.pathname === '/memory/') {
      try { res.end(JSON.stringify(getMemoryData())); }
    ...
    if (url.pathname.startsWith('/memory/entry/')) {
      ...
      if (parts.length >= 2) {
        ...
        else { res.writeHead(404); res.end(JSON.stringify({ error: 'Entry not found' })); }
        return;
      }
    }
