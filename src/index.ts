...
function render(
  ...
  if (backBtn) {
    backBtn.addEventListener('click', () => {
      const mm = root.parentNode as any;
      if (view.kind === 'entry') {
  ...
  for (const el of Array.from(root.querySelectorAll<HTMLElement>('[data-project][data-file]'))) {
    el.addEventListener('click', () => {
      ...
      if (!entry) return;
      loadEntry(root, ctx, data, project, entry, root.parentNode as any);
    });
  }
...
async function loadEntry(
  ...
  try {
    ...
    const c = themeColors(ctx.theme === 'dark');
    render(root, ctx, data, false, (err as Error).message, mm._mmView ?? { kind: 'list' });
  }
}
...
export function mount(container: HTMLElement, pluginApi: PluginAPI): void {
  ...
  const mm = container as any;
  if (!mm._mmView) mm._mmView = { kind: 'list' };
  ...
  root.addEventListener('click', (e) => {
    const btn = (e.target as HTMLElement).closest('#mm-refresh-btn');
    if (btn) {
      firstLoad = true;
      loadData();
    }
